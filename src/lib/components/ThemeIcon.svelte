<script lang="ts">
  ...
  const setTheme = (theme: string, oldTheme: string = ""): void => {
    ...
    isChecked = theme === "dracula";
  };

  const syncThemeToggle = (currentTheme: string) => {
    isChecked = currentTheme === "dracula";
  ...
  const onStorageChange = (event: StorageEvent) => {
    if (
      ...
      syncThemeToggle(event.newValue);
    }
  };
  ...
  onDestroy(() => {
    if (window) window.removeEventListener("storage", onStorageChange);
  });
</script>
